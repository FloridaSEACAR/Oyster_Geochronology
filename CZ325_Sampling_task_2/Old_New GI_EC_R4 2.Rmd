---
title: "New Goose Island EC Reef 4"
...
"Utilities.R" is part of DEP AGREEMENT NO. CZ325, Deliverable 1a (Modeling approach to age and time-averaging estimation)
```{r}
source("Utilities.R")
```
...
```{r}
Durham_GAS_April24_2023 %>% filter(grepl("GI-EC_R4H3S1", name)) -> GI_EC_R4_New
```

Looks like Station 8842
```{r}
SpecNames_Xwalk %>% filter(grepl("GI-EC_R4H3S1", Sample_ID))
```
...
GI_EC_R4_Old %<>% filter(Station == 8848) %>% select(Cat_No,F14C_corr,F14C_corr_sd,Depth,Station)
...
```
Remember these are from GI-EC_R4H3S1
total of 25 shells
```{r}
...
```{r}
Old_posteriors = GI_EC_R4_Old_posteriors
Old_fractions = GI_EC_R4_Old
Goose_Island_East_Cove_R4H3S1_Old_results = data.frame(n = nrow(Old_fractions), cpe = CPE(Old_posteriors)$cpe
...
```{r}
write_rds(Goose_Island_East_Cove_R4H3S1_Old_results, "Goose_Island_East_Cove_R4H3S1_Old_Data_New_Curve_results.RDS")
```


Get the original estimates from the spreahsheet -- note that these are from the original calibration curve
```{r}
...
HOBSGeo_Apps_202208_final.DR1 %>% filter(Station == 8848  & Depth == "15-25cm" ) %>% select(N, Median,IQR_TAV,IQR_CPE) %>% data.frame() -> GI_EC_R4_original_estimates

```
Original Data, Original Curve
```{r}
write_rds(GI_EC_R4_original_estimates, "GI_EC_R4_original_estimates.RDS")
```


```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(-Median) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2) + geom_smooth(color = "orange") + facet_wrap(~variable) + ggtitle("New Goose Island/East Cove Reef 4")
```

```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,Median) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_jitter(data = GI_EC_R4_original_estimates, aes(x = N, y = Median,color = "Original Curve"),  size = 2) + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = Median, color = "New Curve"), size = 2) + ggtitle("New Goose Island/East Cove Reef 4") + ylab("Median")
```

```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,cpe) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_point(data = GI_EC_R4_original_estimates, aes(x = N, y = IQR_CPE,color = "Original Curve"), size = 2)  + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = cpe, color = "New Curve"), size = 2) + ggtitle("New Goose Island/East Cove Reef 4") + ylab("IQR_CPE")
```

```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,tav_iqr) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_jitter(data = GI_EC_R4_original_estimates, aes(x = N, y = IQR_TAV,color = "Original Curve"),  size = 2) + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = tav_iqr, color = "New Curve"), size = 2)  + ggtitle("New Goose Island/East Cove Reef 4") + ylab("IQR_TAV")
```

```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,cr_0.95) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = cr_0.95, color = "New Curve"), size = 2) + ggtitle("New Goose Island/East Cove Reef 4") + ylab("cr_0.95")
```


```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,tav_hdi) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = tav_hdi, color = "New Curve"), size = 2) + ggtitle("New Goose Island/East Cove Reef 4") + ylab("tav_hdi")
```

```{r}
Goose_Island_East_Cove_R4H3S1_results %>% select(n,hdi_of_hdi) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = Goose_Island_East_Cove_R4H3S1_Old_results, aes(x = n, y = hdi_of_hdi, color = "New Curve"), size = 2)  + ggtitle("New Goose Island/East Cove Reef 4") + ylab("hdi_of_hdi")
```
...
hdi_of_hdi.tmp = Goose_Island_East_Cove_R4H3S1_results %>% group_by(n) %>% dplyr::summarize( hdi_of_hdi_Bias = Bias(hdi_of_hdi, pop_values$hdi_of_hdi), hdi_of_hdi_SD = StandardDeviation(hdi_of_hdi, pop_values$hdi_of_hdi), hdi_of_hdi_CV =  StandardDeviation(hdi_of_hdi, pop_values$hdi_of_hdi)/pop_values$hdi_of_hdi)%>% select(-n)

Goose_Island_East_Cove_R4H3S1_summary = round(cbind(cpe.tmp,tav_iqr.tmp,cr_0.95.tmp,tav_hdi.tmp), 3)
Goose_Island_East_Cove_R4H3S1_summary
```
