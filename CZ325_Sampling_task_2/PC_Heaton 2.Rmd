...
```
```{r}
PC_All_NP_results %>% select(-Median) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.5, size = 0.5) + geom_smooth(color = "orange") + facet_wrap(~variable) + ggtitle("Pellicer Creek")
```