...
```{r}
All_NP  %>% ggplot(aes(x = n, y = Median)) + geom_jitter(width = 0.2) + geom_smooth(color = "orange") + facet_wrap(~Locality)  + ggtitle("NP Median")
```
...
All_ad_hoc_results %>% filter(Median <= 2023) %>% ggplot(aes(x = n, y = Median)) + geom_jitter(width = 0.2) + geom_smooth(color = "orange") + 
  ...
  facet_wrap(~Locality)  + ggtitle("TAV Median")
```
