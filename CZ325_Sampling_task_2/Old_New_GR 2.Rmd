...
```{r}
Old_posteriors = GR_Old_posteriors
Old_fractions = GR_Old
GR_Old_results = data.frame(n = nrow(Old_fractions), cpe = CPE(Old_posteriors)$cpe
...
```{r}
write_rds(GR_Old_results, "Guana_River_Old_Data_New_Curve_results.RDS")
```
...
HOBSGeo_Apps_202208_final.DR1 %>% filter(Station == 8855) %>% select(N, Median,IQR_TAV,IQR_CPE) %>% data.frame() -> GR_original_estimates
```

Original Data, Original Curve
```{r}
write_rds(GR_original_estimates, "GR_original_estimates.RDS")
```

```{r}
Guana_River_R3H1S1_results %>% select(-Median) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + facet_wrap(~variable) + ggtitle("Guana River")
```

```{r}
Guana_River_R3H1S1_results %>% select(n,Median) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_point(data = GR_original_estimates, aes(x = N, y = Median,color = "Original Curve"), size = 2) + geom_point(data = GR_Old_results, aes(x = n, y = Median, color = "New Curve"), size = 2) + ggtitle("Guana River") + ylab("Median")
```

```{r}
Guana_River_R3H1S1_results %>% select(n,cpe) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_point(data = GR_original_estimates, aes(x = N, y = IQR_CPE,color = "Original Curve"), size = 2)  + geom_point(data = GR_Old_results, aes(x = n, y = cpe, color = "New Curve"), size = 2) + ggtitle("Guana River") + ylab("IQR_CPE")
```
```{r}
Guana_River_R3H1S1_results %>% select(n,tav_iqr) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange") + geom_point(data = GR_original_estimates, aes(x = N, y = IQR_TAV,color = "Original Curve"), size = 2) + geom_point(data = GR_Old_results, aes(x = n, y = tav_iqr, color = "New Curve"), size = 2)  + ggtitle("Guana River") + ylab("IQR_TAV")
```
```{r}
Guana_River_R3H1S1_results %>% select(n,cr_0.95) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = GR_Old_results, aes(x = n, y = cr_0.95, color = "New Curve"), size = 2)  + ggtitle("Guana River") + ylab("cr_0.95")
```

```{r}
Guana_River_R3H1S1_results %>% select(n,tav_hdi) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = GR_Old_results, aes(x = n, y = tav_hdi, color = "New Curve"), size = 2)  + ggtitle("Guana River") + ylab("tav_hdi")
```

```{r}
Guana_River_R3H1S1_results %>% select(n,hdi_of_hdi) %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.1, size = 0.5) + geom_smooth(color = "orange")  + geom_point(data = GR_Old_results, aes(x = n, y = hdi_of_hdi, color = "New Curve"), size = 2)  + ggtitle("Guana River") + ylab("hdi_of_hdi")
```
...
hdi_of_hdi.tmp = Guana_River_R3H1S1_results %>% group_by(n) %>% dplyr::summarize( hdi_of_hdi_Bias = Bias(hdi_of_hdi, pop_values$hdi_of_hdi), hdi_of_hdi_SD = StandardDeviation(hdi_of_hdi, pop_values$hdi_of_hdi), hdi_of_hdi_CV =  StandardDeviation(hdi_of_hdi, pop_values$hdi_of_hdi)/pop_values$hdi_of_hdi)%>% select(-n)

Guana_River_R3H1S1_summary = round(cbind(cpe.tmp,tav_iqr.tmp,cr_0.95.tmp,tav_hdi.tmp,hdi_of_hdi.tmp), 3)
```
