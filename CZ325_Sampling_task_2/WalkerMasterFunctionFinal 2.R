...
WalkerBivarDirichlet <- function(x, xsig, 
                                    ...
                                    slicew = 1000, m = 10, calcurve, kstar = 10, sensibleinit = TRUE) {
  ...
  IntCalyrgrid <- FindCal(seq(min(caltheta), max(caltheta), by = 1), calmu, caltheta, calsig)
  ...
  for(MHiter in 1:niter) {
    ...
    for(k in 1:nobs) {
      theta[k] <- SliceSample(TARGET = thetaloglikfast, x0 = theta[k],
                              ...
                              mucalallyr = mucalallyr, sigcalallyr = sigcalallyr, caltheta = caltheta)
    }
