---
title: "Heaton's Method on New Data from Goose Island/East Cove Reef 4"
...
GI_EC_R4_Old %<>% filter(Station == 8848) %>% select(Cat_No,F14C_corr,F14C_corr_sd)
names(GI_EC_R4_Old) = c("name", "F14C_corr","F14C_corr_sd")
```
...
GI_EC_R4_New %<>% select(name, F14C_corr,F14C_corr_sd)
```
...
GI_EC_R4_All  = as.data.frame(rbind(GI_EC_R4_Old,GI_EC_R4_New))

GI_EC_R4_All_names = GI_EC_R4_All$name
GI_EC_R4_All_NP_results = data.frame()
set.seed(1234)
system.time(
for( n in seq(3,length(GI_EC_R4_All_names), by = 1)){ 
     for( ii in 1:30){ 
        random_sample = sample(GI_EC_R4_All_names, size = n, replace = FALSE)
        # get the factions for these samples
        tmp = GI_EC_R4_All %>% filter(name %in% random_sample)
        ...
        GI_EC_R4_All_NP_results = rbind(GI_EC_R4_All_NP_results, data.frame( n = n, NP_IQR = Posterior_IQR(NP), NP_95 = Posterior_IQR(NP, lo = 0.025, hi = 0.975), NP_HDI = HDI(NP), Median = Median(NP)))
     }
}
)
```
...
```{r}
write_rds(GI_EC_R4_All_NP_results,"GI_EC_R4_All_NP_results.RDS")
```

```{r}
GI_EC_R4_All_NP_results %>% reshape2::melt("n") %>% ggplot(aes(x = n, y = value)) + geom_jitter(width = 0.2, alpha =0.5, size = 0.5) + geom_smooth(color = "orange") + facet_wrap(~variable) + ggtitle("Goose Island/East Cove Reef 2")
```