...
thetaloglik <- function(z, prmean, prsig, c14obs, c14sig, calmu, caltheta, calsig) {
  ...
  if(z < min(caltheta) | z > max(caltheta)) return(-Inf)
...
# A fast version which assume calibration curve is constant on a year scale
thetaloglikfast <- function(z, prmean, prsig, c14obs, c14sig, mucalallyr, sigcalallyr, caltheta) {
  yr <- floor(z) # Assume the cal likelihood is constant on year scale
  # Return -Inf if outside range
  if(yr < min(caltheta) | yr > max(caltheta)) return(-Inf)
...
plotindpost <- function(MCMCoutput, ident, y, er, calcurve, PresentDay,
                        nburn = NA, nbreaks = NA) {
  ...
  plot(calcurve$Year, calcurve$c14age, col = "blue", 
       ylim = range(calcurve$c14age) + c(-2,2) * max(calcurve$c14sig), 
       xlim = range(calcurve$Year),
       ...
       xaxs = "i", yaxs = "i")
  lines(calcurve$Year, calcurve$ub, lty = 2, col = "blue")
  lines(calcurve$Year, calcurve$lb, lty = 2, col = "blue")
  
  year_range <- range(calcurve$Year)
  # Plot the 14C determination on the y-axis 
  yfromto <- seq(c14age - 3 * c14sig, c14age + 3 * c14sig, length  = 100)
  radpol <- cbind( c(0, dnorm(yfromto, mean = c14age, sd = c14sig), 0), 
                   c(min(yfromto), yfromto, max(yfromto)))
  radpol[,1] <- radpol[,1] * 0.1 * (year_range[2] - year_range[1]) / max(radpol[,1])
  radpol[,1] <-  year_range[1] + radpol[,1]
  ...
  temphist <- hist(PresentDay - theta, breaks = nbreaks, plot = FALSE)
  hist(PresentDay - theta, prob = TRUE, breaks = nbreaks, xlim = range(calcurve$Year),
